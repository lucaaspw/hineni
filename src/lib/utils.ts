...
export function getWeekStart(date: Date = new Date()): Date {
  ...
  const diff = d.getDate() - day; // Diferença até o domingo
  const weekStart = new Date(d.setDate(diff));
  weekStart.setHours(0, 0, 0, 0); // Zerar horas para comparar apenas datas
  return weekStart;
}
...
export function generateRotatedScales(
  ...
  while (repeatedCombinations.length < neededCombinations) {
    repeatedCombinations.push([...normalizedCombinations[comboIndex % normalizedCombinations.length]]);
    comboIndex++;
  }
  ...
  for (let i = 0; i < neededCombinations; i++) {
    ...
    if (usedRecently.size > recentWindow) {
      ...
      if (finalCombinations.length >= recentWindow) {
        const oldestKey = finalCombinations[finalCombinations.length - recentWindow].join(',');
